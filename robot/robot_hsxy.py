...
class robot:
	...
	def loginGame(self):
		...
		self.entryRoles(1,self.getStr(struct.unpack('!I',body[4:8])))
		self.loopDoAction()
	#选择角色进入游戏
	def entryRoles(self,sn,roleid):
		...
		pass
	#进入场景
	def entrySceene(self):
		Cmd = 12005
		sendPackInfo = struct.pack('!I',2040)
		sendPackInfo = self.pack(Cmd,sendPackInfo)
		self.sendPack(sendPackInfo)
		rs = self.recvPack()
		pass
	...
	def loopDoAction(self):
		...
		# self.runRobot()
		# self.sendHeart()
		# #self.getScene()		#获取相邻场景
		# self.entrySceene()
		# self.getUserInfo()	#获取玩家信息
		# self.jzScene()
		# time.sleep(8)
		# self.chart()
		# for i in range(4):
	...
	def get_shop_list(self):
		Cmd = 15013
		packInfo = struct.pack('!H',1) #shop
		packInfo = packInfo + struct.pack('!H',2)
		sendPackInfo = self.pack(Cmd,packInfo)
		self.sendPack(sendPackInfo)
		self.recvPack()
	def runRobot(self):
		Cmd = 10006
		X = 65
		Y = 23
		sendPackInfo = struct.pack('!H',65)+struct.pack('!H',23)
		sendPackInfo = self.pack(Cmd,sendPackInfo)
		self.sendPack(sendPackInfo)
		self.recvPack()
		pass
	...
	def showMsg(self,msg,out=None):
		if self.output:
			charStr = self.output.GetValue()
			#print msg
			self.output.SetValue(charStr+str(msg)+"\n")
		else:
			print msg
#对外提供的模块调用接口
def getRobot():
